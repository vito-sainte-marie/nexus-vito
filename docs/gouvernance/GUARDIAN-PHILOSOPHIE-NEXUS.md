...
## Rapatriement sur la ligne `production` (16/09/2026)

Ce document et sa garde vivaient sur `config-par-environnement`, une ligne qui
a divergé de `production` de plusieurs centaines de commits. Ils sont repris
ici **fichier par fichier**, sans fusion de branche : la doctrine, l'outil et
son épreuve, rien d'autre.

**Deux références de ce document ne trouvent pas leur cible sur cette ligne.**
`outils/handoff.js` et `docs/handoff/PROTOCOL.md` n'y existent pas — le
registre Handoff n'a jamais été rapatrié sur `production`. En conséquence :

- la première puce de « Où il s'applique » (section `## Guardian Philosophie
  NEXUS` obligatoire dans une demande Handoff, bloquante via
  `validerGuardianPhilosophie`) **décrit un mécanisme absent ici** ; elle est
  conservée telle quelle parce qu'elle reste vraie là où le registre existe,
  et parce qu'amputer une doctrine pour la faire coller à son environnement
  est précisément la dérive que le critère 4 interdit ;
- la seconde puce (gates humaines) et la totalité des neuf critères
  s'appliquent sans réserve ;
- la garde mécanisée, elle, **ne dépend d'aucun de ces fichiers** : elle ne lit
  que `docs/gouvernance/GUARDIAN-PHILOSOPHIE-NEXUS.md` et les écrans
  `NEXUS-*.html` de la racine. Vérifié, pas supposé.

**Mesure refaite sur cette ligne, pas recopiée** — la règle QA-002 vaut aussi
pour un rapatriement : **7 signalements sur 61 écrans**. Le dépôt n'est pas le
même (61 écrans ici contre 62 là-bas) ; le chiffre de 7 coïncide, il n'est pas
hérité. Les sept sont réels et relus : `NEXUS-FDJ-Manager-v1.html` (×2),
`NEXUS-Parametres-Inventaire-v1.html`, `NEXUS-Paye-v1.html` (×3),
`NEXUS-Tracabilite-v1.html`. Aucun n'est corrigé par ce lot : la garde est
consultative, et corriger sept écrans sous couvert de câbler une garde ferait
exactement ce que le critère 7 proscrit.

**Ce qui est câblé dans la CI de cette ligne**, et rien de plus : une étape de
rapport non bloquante (`|| true`), conforme à l'arbitrage Q75. Son épreuve
propre n'a PAS besoin d'étape dédiée ici : `run-tests.js` ramasse tout
`test_*.js` de la racine, et l'étape « Comparer aux échecs connus » est stricte
dans les deux sens — une épreuve qui casserait apparaîtrait en `NOUVEAUX` et
arrêterait la CI. Une seconde étape ne rendrait pas la garde plus bloquante,
elle ferait un deuxième endroit à maintenir.
