...
preuves:
  ...
  - id: reconciliation-verifiee-independamment
    classe: VERIFIED
    valeur: 290a217-fe36a8e-parent-diffs-vides-fonctions-4e1b4c3-confirmees-par-git
  ...
  - id: plan-construit-et-teste
    classe: VERIFIED
    valeur: merge-4-blocs-plus-consolidation-predicat-manager-763-lignes-session-origine
  - id: harnais-1-vert
    classe: VERIFIED
    valeur: test_regularisation_manager_20260916-harnais-realigne-1-24-sur-24-session-origine
  - id: harnais-2-vert
    classe: VERIFIED
    valeur: test_cloture_services_obsoletes_20260916-harnais-realigne-1-14-sur-14-session-origine
  - id: diff-mesure
    classe: VERIFIED
    valeur: 466-insertions-9-suppressions-contre-candidate-actuelle-un-seul-fichier
  - id: classification
    classe: VERIFIED
    valeur: RESTAURATION_DE_COMPORTEMENT_EXISTANT
...
## 0. Reconciliation de ce dépôt — travail hérité, vérifié indépendamment, pas dupliqué

Ce `request-11.md` a été produit par une session Claude concurrente (branche
`claude/issue-28-20260923-2329`, commit `69eb1f8`), sur le même parent canonique
(`a730441`) que celui atteint par cette session au moment de son réveil. Conformément à la
consigne explicite « sans créer de doublon concurrent », cette session n'a pas relancé la même
investigation : elle a lu et **vérifié indépendamment** chacune des affirmations factuelles
ci-dessous avant de reconcilier ce fichier vers le rail canonique par le mécanisme prévu
(`outils/handoff.js demande`), plutôt que par une écriture directe.

Vérifications indépendantes effectuées dans cette session (commandes Git réelles, pas relues sur
la confiance) :
- `290a217` a bien pour unique parent `fe36a8e` (`git show -s --format=%P`).
- `git diff fe36a8e:nexus-auth.js origin/production:nexus-auth.js` → vide (0 ligne) ; les deux
  fichiers font 932 lignes.
- `git diff 290a217:nexus-auth.js a730441:nexus-auth.js` → vide ; `git diff
  290a217:nexus-auth.js origin/rebuild/carburants-65-20260922:nexus-auth.js` → vide ; les trois
  fichiers font 305 lignes.
- `4e1b4c3` est bien ancêtre de `origin/production` (`git merge-base --is-ancestor`) et définit
  bien `nexusEstManager`, `nexusReglesPilote`, `nexusAppliquerCloturePilote`,
  `nexusCloturerServicesObsoletes`, `nexusServicesOuvertsDuSite`,
  `nexusRegulariserServicesObsoletes` (grep sur `git show 4e1b4c3:nexus-auth.js`).
- `7afbf27` et `e6161ac` sont bien ancêtres de `origin/production` ; `origin/production:nexus-auth.js`
  (932 lignes, identique à `fe36a8e`) porte toujours aujourd'hui les cinq fonctions ci-dessus ainsi
  que `nexusJourDansFuseau`/`nexusFuseauSite`.
- Le rail (`a730441:nexus-auth.js`) porte bien exactement deux occurrences résiduelles de
  `employee.role==='manager'||employee.role==='gerant'` (lignes 180 et 261 de
  `nexusPointageArriveeManquant`/`nexusPriseDePosteManquante`), confirmant le besoin de
  consolidation du §4.5 ci-dessous.

Toutes les affirmations vérifiables sans reconstruire le fichier fusionné se confirment à
l'identique. Le plan de fusion lui-même (763 lignes, 38/38 assertions) n'a pas été rejoué dans
cette session — il avait déjà été prouvé en zone jetable, supprimée avant dépôt, par la session
d'origine ; le rejouer une seconde fois aurait été la duplication que ce réveil interdit
explicitement. Aucune écriture sur `nexus-auth.js` n'a eu lieu dans l'une ou l'autre session.
...
## 5. Preuve — construite et exécutée réellement, en zone jetable, jamais poussée (session d'origine)
...
aucune écriture sur aucune branche. Étapes réellement exécutées dans la session d'origine :
...
5. Toute la zone de travail (fichiers copiés dans le répertoire du lot pour l'exécution, script de
   construction) a été supprimée avant la fin de la session d'origine — `git status` confirmé propre sur
   `docs/handoff/lots/NEXUS-CONTINUITE-TERRAIN-2-20260922/` et sur l'arbre entier avant ce dépôt.

Cette session (réveil du 2026-09-23 23:50) n'a pas rejoué ces 38 assertions : elle a vérifié
indépendamment chaque fait statique reconstructible par Git (§0), sans reconstruire le fichier
fusionné une seconde fois, pour ne pas dupliquer un travail déjà prouvé et déjà décrit
exhaustivement ici.
...
## 7. Classification demandée par le mandat de réveil

**`RESTAURATION_DE_COMPORTEMENT_EXISTANT`.**

Les trois fonctions nommées par `request-10.md` (`nexusServicesOuvertsDuSite`,
`nexusCloturerServicesObsoletes`, `nexusAppliquerCloturePilote`) ne sont ni un contrat de test sans
implémentation, ni une contradiction canonique : elles existent, aujourd'hui, en Production
(`origin/production:nexus-auth.js`, ancêtre `4e1b4c3` du 16/09/2026), avec leur propre historique de
commits et de tests, actives et exploitées. Leur absence du rail `handoff-continuite-20260920` et de
la candidate n'est pas une décision — c'est un accident de transport : le portage mécanique `290a217`
a remplacé, en un seul geste, tout `nexus-auth.js` de la candidate (qui venait de recevoir la tête de
`production` via `fe36a8e`) par la version du rail, laquelle n'a jamais fusionné ces deux
fonctionnalités validées séparément le 16/09 et les 18-19/09/2026. Restaurer ces blocs n'invente
aucune règle métier/UX/RLS/rôle nouvelle : ceci reproduit, à l'identique, du comportement déjà
approuvé et en service.
...
- **QA/Regression** : preuve par échec puis succès mesuré (§5.1-5.2), pas seulement par succès — le
  ...
  aucune assertion affaiblie ou contournée. Cette session a revérifié indépendamment les faits Git
  statiques sous-jacents (§0), sans relire sur la confiance.
- **Bible/Philosophie** : préférence donnée à nommer précisément ce qui est exclu (§4, §6) plutôt qu'à
  ...
  décision ; aucun doublon concurrent créé — travail hérité vérifié et reconcilié plutôt que refait.
