...
```
git fetch origin claude/issue-28-20260907-1232 config-par-environnement
git checkout -b lot/carburants-optimisation-camion-decision2 origin/config-par-environnement
git checkout origin/claude/issue-28-20260907-1232 -- \
  nexus-carburant-commande-moteur.js \
  test_carburant_commande_p0_traversee_reliquat_20260907.js
node test_carburant_commande_p0_traversee_reliquat_20260907.js   # doit rester 2/2
node run-tests.js carburant_commande                             # doit rester vert
git commit -m "handoff: correctif traversée reliquatArrondi (decision-2) + test P0"
git push origin lot/carburants-optimisation-camion-decision2:config-par-environnement
```

(Seul `nexus-carburant-commande-moteur.js` + le nouveau test portent un changement réel ; `nexus-carburant-commande-donnees-core.js`/`nexus-carburants-p0-fixes.js` n'ont été recopiés dans ce checkout que pour rendre les tests exécutables depuis une base `main` stale — ils sont déjà identiques sur `config-par-environnement`, donc absents de la commande ci-dessus.)

## 6) Preuve UI/navigateur NEXUS Test — NON SATISFAITE, explicitement

Conformément à `decision-2.md` §D, l'ordre est : intégrer d'abord sur `config-par-environnement`, déployer/servir sur NEXUS Test, **puis seulement** exécuter la recette navigateur. L'intégration (§5) n'a pas eu lieu — exécuter une recette navigateur maintenant testerait la version non corrigée déjà déployée sur `nexus-test`, ce que `decision-2.md` interdit explicitement (« ne jamais fabriquer une preuve sur une version qui ne contient pas le correctif »). Ce point reste **`HUMAN`/non satisfait**, le lot reste ouvert.
