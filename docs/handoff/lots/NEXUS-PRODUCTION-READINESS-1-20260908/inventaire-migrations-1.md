...
| 3 | `20260904190027_site_unique_shifts_mission_catalog` | écriture au déploiement + schéma (drop default) | **corrigées** — `shifts.site_id := site`, `mission_catalog.site := site_id`, uniquement où les deux colonnes divergent | nombre de lignes réellement affectées en Production : voir mesures ci-dessous |
...
libellé retouché à la main en Production, par exemple). Impact réel : à
chiffrer champ par champ via `comparaison-seed-referentiel-advisor.sql`
(même répertoire), une requête strictement `SELECT`.
...
même employé). Voir mesures ci-dessous pour le nombre de lignes réellement
...
## Mesures Production en lecture seule — canoniques

Ces chiffres sont enregistrés dans `mesures-production-lecture-seule-1.md`
(même répertoire), déposé directement sur `config-par-environnement` par le
NEXUS Orchestrator via le connecteur Supabase déjà autorisé, sans nouveau
rôle/secret/accès, sur la candidate figée
...
donc reprises ici telles que déposées canoniquement, pas comme un fait que
cette session aurait elle-même constaté. Elles répondent exactement aux deux
points d'attention posés ci-dessus.
...
plage de forte activité (voir fenêtre de déploiement,
`plan-reparation-rollback-1.md`).
...
- Il ne mesure rien de nouveau en Production lui-même : les valeurs
  ci-dessus sont reprises telles que déposées canoniquement par
  l'Orchestrator (aucun identifiant/réseau Production disponible dans ce
  canal GitHub Issue — cohérent avec la restriction documentée depuis le
  06/09/2026 sur ce workflow).
- Il ne construit pas PREPROD, ne fixe pas les critères `Prêt pour
  Production` dans NEXUS Live, et ne rédige pas le plan de réparation en
  avant détaillé — voir les fichiers dédiés du même répertoire.
