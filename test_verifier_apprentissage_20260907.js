...
// Ajoutée le 07/09/2026 : une mutation supprimant le contrôle de schéma
// survivait à toute la suite. Un registre sans `schema` déclaré, ou déclarant
// un autre format, serait lu par un outil qui croit connaître sa forme —
// c'est ainsi qu'on interprète un champ absent comme une valeur par défaut.
t('validerRules refuse un schéma absent ou étranger', () => {
  const regle = { id: 'ENV-001', scope: ['development'], trigger: 't', module: 'all', severity: 'blocking', rule: 'r', source: 's' };
  for (const schema of [undefined, 'nexus-rules/2', 'autre-chose']) {
    const doc = { rules: [regle] };
    if (schema !== undefined) doc.schema = schema;
    const f = fichierTemp(JSON.stringify(doc));
    const { erreurs } = validerRules(f);
    fs.unlinkSync(f);
    assert.ok(erreurs.some(e => /schema/.test(e)),
      `schema ${JSON.stringify(schema)} doit être refusé, erreurs obtenues : ${JSON.stringify(erreurs)}`);
  }
});
