...
-- le 04/09/2026. Version 20260831102512 · inventaire_transfert_localise_destination_initialisee
--
...
declare
  ...
  v_mouvement_id uuid;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUISE'; end if;

  select * into v_employee from employees where id = auth.uid();
  if not found or v_employee.site_id is distinct from p_site then raise exception 'SITE_NON_AUTORISE'; end if;
  if v_employee.role not in ('manager','gerant') then raise exception 'ROLE_NON_AUTORISE'; end if;
  ...
  if p_quantite_base is null or p_quantite_base <= 0 then raise exception 'QUANTITE_INVALIDE'; end if;
  ...
  limit 1
  for update;
  if not found then raise exception 'STOCK_SOURCE_NON_INITIALISE'; end if;

  perform 1
  from inventaire_stock_localise_releves
  where site = p_site and produit_id = p_produit_id and zone_id = p_zone_destination_id
  limit 1;
  if not found then raise exception 'STOCK_DESTINATION_NON_INITIALISE'; end if;
