...
(async () => {
  ...
  assert.ok(testJs.includes("zonesAutorisees = data.map(z => z.code).filter(Boolean)"), 'zones dérivées des règles de mission');

  console.log('OK — test Renfort accessible pendant le quart, ciblé par zone et strictement non persistant.');
