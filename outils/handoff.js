...
// Types dont la demande touche l'orchestration/l'architecture : le Guardian
// Philosophie NEXUS (docs/gouvernance/GUARDIAN-PHILOSOPHIE-NEXUS.md) doit
// avoir répondu par écrit, dans le corps, à « pourquoi un humain doit-il
// intervenir ici ? » avant que le lot soit recevable.
const TYPES_GUARDIAN_PHILOSOPHIE_REQUIS = ['architecture-and-implementation-request'];
const erreurs = [];
...
// Même chose que echanges(), mais lue depuis une réf git (ex. origin/config-par-environnement)
// plutôt que depuis le disque. `git fetch` seul ne change pas les fichiers de
// la copie de travail : un veiller() qui relisait le disque après un fetch ne
// détectait donc jamais rien de nouveau tant qu'aucun checkout/reset n'avait
// eu lieu. Lire directement la réf distante rend la détection d'événement
// possible sans exiger d'écriture dans la copie de travail.
function echangesRef(ref, lot, genre) {
  const prefixe = genre === 'request' ? 'request-' : 'decision-';
  let sortie;
  try { sortie = git('ls-tree', '--name-only', ref, `docs/handoff/lots/${lot}/`); } catch (e) { return []; }
  return sortie.split('\n').filter(Boolean).map(p => path.basename(p)).filter(f => f.startsWith(prefixe) && f.endsWith('.md')).map(f => ({ fichier: f, seq: parseInt(f.slice(prefixe.length, -3), 10) })).filter(e => Number.isInteger(e.seq) && e.seq > 0).sort((a, b) => a.seq - b.seq);
}
...
// Garde Philosophie NEXUS (docs/gouvernance/GUARDIAN-PHILOSOPHIE-NEXUS.md) :
// une demande d'architecture/orchestration doit répondre par écrit à
// « pourquoi un humain doit-il intervenir ici ? ». Vérifié sur la présence de
// la section, pas sur son contenu — juger la qualité de la réponse reste une
// question de jugement, pas de vocabulaire, comme pour token_mode.
function validerGuardianPhilosophie(ou, env, corps, fichier) {
  if (!env.type || !TYPES_GUARDIAN_PHILOSOPHIE_REQUIS.includes(env.type)) return;
  if (!/^##\s+Guardian Philosophie NEXUS\s*$/m.test(corps || '')) {
    bloquant(`${ou} : type ${JSON.stringify(env.type)} exige une section "## Guardian Philosophie NEXUS" répondant à « pourquoi un humain doit-il intervenir ici ? » (docs/gouvernance/GUARDIAN-PHILOSOPHIE-NEXUS.md) — absente`, 'GUARDIAN_PHILOSOPHIE_MANQUANTE', fichier);
  }
}
  ...
  for (const lot of lots()) {
    ...
    for (const e of decisions) {
      ...
      if (!env.in_reply_to) bloquant(`${ou} : in_reply_to manquant`, 'IN_REPLY_TO_MANQUANT', e.fichier); else { const vise = path.basename(String(env.in_reply_to).trim()), cible = demandes.find(d => d.fichier === vise); if (!cible) bloquant(`${ou} : in_reply_to ${JSON.stringify(env.in_reply_to)} ne désigne aucune demande de ce lot`, 'IN_REPLY_TO_INCONNU', e.fichier); else { reponses.push({ decision: e, viseSeq: cible.seq }); if (cible.seq < e.seq) bloquant(`${ou} : in_reply_to désigne ${vise} (rang ${cible.seq}), antérieur au rang de la décision (${e.seq})`, 'IN_REPLY_TO_ANTERIEUR', e.fichier);
        // Anti-boucle : une décision qui porte le même auteur que la demande à
        // laquelle elle répond serait un auto-arbitrage — l'agent qui demande
        // ne peut pas être celui qui décide, faute de quoi le protocole ne
        // protège plus rien. Non dérogeable : c'est la même famille que les
        // refs protégées, un invariant de sécurité du protocole lui-même.
        if (env.author && auteurDemande[vise] && env.author === auteurDemande[vise]) bloquant(`${ou} : auteur ${JSON.stringify(env.author)} identique à celui de ${vise} — auto-arbitrage refusé`, 'AUTO_ARBITRAGE', e.fichier);
      } }
    }
...
function validerEtat() {
  ...
  const actifs = Object.entries(etat.lots).filter(([, v]) => v.statut === 'ATTENTE_DECISION').map(([k]) => k); if (actifs.length > 1) bloquant(`STATE.json : ${actifs.length} lots en attente (${actifs.join(', ')}) — un seul lot actif dans cette version`, 'PLUSIEURS_LOTS_ACTIFS'); if (etat.lot_actif && !etat.lots[etat.lot_actif]) bloquant(`STATE.json : lot_actif ${etat.lot_actif} absent de lots`);
  for (const [lot, v] of Object.entries(etat.lots)) { const demandes = echanges(lot, 'request'), decisions = echanges(lot, 'decision'); if (v.derniere_demande) { if (!demandes.find(d => d.fichier === v.derniere_demande)) bloquant(`STATE.json : ${lot}.derniere_demande ${v.derniere_demande} absente du registre`); else if (v.derniere_demande !== dernier(demandes).fichier) bloquant(`STATE.json : ${lot}.derniere_demande ${v.derniere_demande} n'est pas la plus récente (${dernier(demandes).fichier})`); } if (v.derniere_decision && v.source_decision === 'registre' && !decisions.find(d => d.fichier === v.derniere_decision)) bloquant(`STATE.json : ${lot}.derniere_decision ${v.derniere_decision} absente du registre`); if (v.consomme_le && !v.commit_decision) bloquant(`STATE.json : ${lot} marqué consommé sans commit_decision`); if (!['ATTENTE_DECISION', 'DECISION_CONSOMMEE', 'CLOS'].includes(v.statut)) bloquant(`STATE.json : ${lot}.statut ${JSON.stringify(v.statut)} hors vocabulaire`); }
...
// Idempotence : la source de vérité n'est jamais un compteur séparé, mais
// STATE.json lui-même (derniere_decision + statut), déjà écrit par
// consommer(). Une brique de veille ne réinvente donc pas un second registre
// à tenir synchrone avec le premier (critère 3, Guardian Philosophie NEXUS) —
// elle relit celui qui existe. « event detected » n'est émis qu'une fois par
// décision distincte ; une décision déjà consommée redonne « already
// consumed » à chaque appel suivant, y compris depuis un watchdog qui
// s'exécute toutes les heures sans jamais dupliquer une action.
function veiller(lot, intervalle) {
  const etat = JSON.parse(fs.readFileSync(ETAT, 'utf8')), v = etat.lots[lot] || {};
  try { git('fetch', '-q', 'origin', BRANCHE_AUTORISEE); } catch (e) {}
  const distante = dernier(echangesRef(`refs/remotes/origin/${BRANCHE_AUTORISEE}`, lot, 'decision'));
  if (!distante) {
    ...
    console.log(`session resumed — ${lot} au statut ${v.statut || 'inconnu'}`); return 0;
  }
  if (v.derniere_decision === distante.fichier && v.statut === 'DECISION_CONSOMMEE') { console.log(`event already consumed — ${lot}/${distante.fichier} (idempotent, aucune action)`); return 0; }
  console.log(`event detected — ${lot}/${distante.fichier}`); return 0;
}
