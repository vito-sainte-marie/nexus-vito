...
function validerPreuves(ou, preuves) {
  ...
  for (const p of preuves) {
    ...
    if (!CLASSES_PREUVE.includes(p.classe)) bloquant(`${ou} : preuve ${p.id} — classe ${JSON.stringify(p.classe)} hors vocabulaire (${CLASSES_PREUVE.join('|')})`);
  }
}

// Une preuve refs-protegees est une empreinte historique, pas un ordre de figer
// main/production à jamais. Elle reste valide si l'empreinte enregistrée est
// toujours un ancêtre de la ref protégée actuelle. Un SHA inconnu, une histoire
// réécrite/force-pushée ou une ref illisible échoue fermé.
function verifierRefsProtegees(ou, valeur) {
  const m = String(valeur || '').trim().match(/^main=([0-9a-f]{7,40})\s+production=([0-9a-f]{7,40})$/i);
  if (!m) {
    bloquant(`${ou} : refs protégées mal formées ${JSON.stringify(String(valeur || '').trim())}`, 'REFS_PROTEGEES');
    return;
  }
  const attendues = { main: m[1], production: m[2] };
  for (const ref of REFS_PROTEGEES) {
    try {
      const historique = git('rev-parse', '--verify', `${attendues[ref]}^{commit}`);
      const courant = git('rev-parse', '--verify', `origin/${ref}^{commit}`);
      try {
        git('merge-base', '--is-ancestor', historique, courant);
      } catch (err) {
        bloquant(`${ou} : ${ref} historique ${attendues[ref]} n'est pas ancêtre de la ref protégée actuelle ${courant.slice(0, 7)} — réécriture ou divergence détectée`, 'REFS_PROTEGEES');
      }
    } catch (err) {
      bloquant(`${ou} : impossible de résoudre ${ref}=${attendues[ref]} ou origin/${ref} — invariant invérifiable`, 'REFS_ILLISIBLES');
    }
  }
}
...
function validerRegistre() {
  for (const lot of lots()) {
    ...
    for (const e of decisions) {
      ...
      if (DECISIONS_LEGACY.includes(env.decision)) bloquant(`${ou} : ${env.decision} est une valeur legacy, lisible dans l'historique v1 mais interdite dans le registre v2 — employer decision + closes.`);
      else if (!DECISIONS_CANONIQUES.includes(env.decision)) bloquant(`${ou} : decision ${JSON.stringify(env.decision)} hors vocabulaire (${DECISIONS_CANONIQUES.join('|')})`, 'DECISION_HORS_VOCABULAIRE', e.fichier);
      ...
      if (!env.in_reply_to) bloquant(`${ou} : in_reply_to manquant`, 'IN_REPLY_TO_MANQUANT', e.fichier);
        ...
        if (!cible) bloquant(`${ou} : in_reply_to ${JSON.stringify(env.in_reply_to)} ne désigne aucune demande de ce lot`, 'IN_REPLY_TO_INCONNU', e.fichier);
          ...
          if (cible.seq < e.seq) bloquant(`${ou} : in_reply_to désigne ${vise} (rang ${cible.seq}), antérieur au rang de la décision (${e.seq})`, 'IN_REPLY_TO_ANTERIEUR', e.fichier);
        }
      }
    }
    for (let i = 1; i < reponses.length; i++) {
      if (reponses[i].viseSeq <= reponses[i - 1].viseSeq) bloquant(`lots/${lot} : ${reponses[i].decision.fichier} répond à une demande déjà arbitrée par ${reponses[i - 1].decision.fichier}`, 'DEMANDE_DEJA_ARBITREE', reponses[i].decision.fichier);
    }
...
function validerEtat() {
  ...
  if (etat.derogations !== undefined) {
    if (!Array.isArray(etat.derogations)) bloquant('STATE.json : derogations doit être une liste');
      ...
      for (const champ of ['fichier', 'regle', 'motif', 'autorise_par', 'le']) if (!d[champ]) bloquant(`STATE.json : dérogation incomplète — ${champ} manquant`);
      if (CODES_NON_DEROGEABLES.includes(d.regle)) bloquant(`STATE.json : ${d.regle} est un invariant de sécurité — aucune dérogation n'est recevable`);
    }
  }
  return etat;
}
...
function verifier(ignorer) {
  ...
  if (etat && etat.lot_actif) {
    ...
    if (d) {
      ...
      if (p && sortie && fs.existsSync(sortie)) {
        ...
        if (m && p.valeur.trim() !== `${m[1]}/${m[2]}`) avertir(`suite déclarée ${p.valeur.trim()}, mesurée ${m[1]}/${m[2]} — lot d'observation : avertissement, pas blocage.`);
      }
    }
  }
  ...
  for (const e of erreurs) {
    ...
    if (d && CODES_NON_DEROGEABLES.includes(e.code)) restantes.push({ ...e, message: `${e.message}\n         (une dérogation existe mais ${e.code} est un invariant de sécurité : elle ne s'applique pas)` });
    else if (d) avertir(`DÉROGATION ${d.regle} sur ${d.fichier} — ${e.message}\n         motif : ${d.motif}\n         autorisée par ${d.autorise_par}, le ${d.le}`);
    else if (ignorer && ignorer.includes(e.code)) avertir(`${e.code} toléré le temps de l'opération en cours — ${e.message}`);
