...
# versionnées (04/09/2026 ; connexion unifiée le 09/09/2026 — voir
# outils/resoudre-connexion-test.sh pour le détail des deux modes).
#
...
# liste des processus) ni écrit dans le dépôt. Deux façons de le fournir,
# voir outils/resoudre-connexion-test.sh :
#   - trousseau macOS (mode historique) :
#       security add-generic-password -a nexus -s nexus-test-db -w
#   - une URL PostgreSQL déjà positionnée dans une variable d'environnement
#     (mode --url-env, pour un rail qui n'a pas de trousseau macOS).
#
# Usage :
#   outils/reconstruire-base-test.sh <project-ref>
#   outils/reconstruire-base-test.sh <project-ref> --url-env NOM_VARIABLE
#
# Ce script REFUSE de s'exécuter sur le projet de production, dans les deux
# modes.
...
REF="${1:-}"
if [ -z "$REF" ]; then
  echo "Usage : $0 <project-ref> [--url-env NOM_VARIABLE]" >&2; exit 2
fi
...
if [ "${REINITIALISER:-oui}" = "oui" ]; then
  ...
  psql --quiet --no-psqlrc -v ON_ERROR_STOP=1 <<'REINIT'
...
for f in "$RACINE"/supabase/migrations/*.sql; do
  ...
  if sortie="$(psql --quiet --no-psqlrc -v ON_ERROR_STOP=1 -f "$f" 2>&1)"; then
    v="${nom%%_*}"; libelle="${nom#*_}"; libelle="${libelle%.sql}"
    psql --quiet --no-psqlrc -c \
