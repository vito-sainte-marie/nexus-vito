...
#
# Le trousseau n'est même TENTÉ que si `NEXUS_TEST_DB_URL` est absente : un
# premier correctif (request-5, 09/09/2026) avait ajouté ce repli sans retirer
# l'appel inconditionnel à `security`, qui s'exécutait donc quand même avant
# d'être ignoré. Reconfirmé et corrigé le même jour : sur un runner qui n'a
# pas de trousseau macOS, avec une URL Test valide déjà fournie, `security`
# ne doit jamais être invoquée du tout.
MDP=""
