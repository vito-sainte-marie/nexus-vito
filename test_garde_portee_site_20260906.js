...
verifier('une aide NON déclarée au registre rend UNKNOWN, jamais SAFE', () => {
  ...
  const r = classer(p, SITE, [{ nom: 'nexus_clients_ecriture_ok', statut: 'CONFORME' }]);
  ...
  assert.strictEqual(classer(p, SITE, [{ nom: 'ma_nouvelle_aide', statut: 'CONFORME' }]).classe, 'SAFE');
});

// ── Q63 (arbitrage NAMED-HELPERS-BEHAVIOR-PROOF) — le registre est une
// dépendance sémantique : une aide DEFAILLANTE ne peut plus laisser SAFE les
// policies qui l'utilisent, et une aide sans preuve CONFORME courante rend
// UNKNOWN, jamais SAFE. Éprouvé par MUTATION, comme l'exige l'arbitrage :
// état accepté vert → mutation volontaire rouge → restauration verte.

verifier('Q63 — une aide CONFORME couvre sa policy en SAFE (état vert)', () => {
  const p = { table: 't', policy: 'p', cmd: 'insert', using: null, withCheck: 'ma_garde(site)' };
  assert.strictEqual(classer(p, SITE, [{ nom: 'ma_garde', statut: 'CONFORME' }]).classe, 'SAFE');
});

verifier('Q63 — mutation : DEFAILLANTE fait rouge, la restauration en CONFORME refait vert', () => {
  const p = { table: 't', policy: 'p', cmd: 'insert', using: null, withCheck: 'ma_garde(site)' };
  const vert = classer(p, SITE, [{ nom: 'ma_garde', statut: 'CONFORME' }]);
  assert.strictEqual(vert.classe, 'SAFE');

  const rouge = classer(p, SITE, [{ nom: 'ma_garde', statut: 'DEFAILLANTE' }]);
  assert.strictEqual(rouge.classe, 'VULNERABLE',
    'une aide que le registre dit défaillante ne peut plus rendre sa policy SAFE sur la seule forme SQL');
  assert.ok(/DEFAILLANTE/.test(rouge.motif) && /ma_garde/.test(rouge.motif),
    'le motif doit nommer l’aide et son statut : ' + rouge.motif);

  const restauree = classer(p, SITE, [{ nom: 'ma_garde', statut: 'CONFORME' }]);
  assert.strictEqual(restauree.classe, 'SAFE', 'restaurer CONFORME doit restaurer SAFE, sans dérive résiduelle');
});

verifier('Q63 — une aide NON_EPROUVEE (preuve comportementale absente) rend UNKNOWN, jamais SAFE', () => {
  const p = { table: 't', policy: 'p', cmd: 'insert', using: null, withCheck: 'ma_garde(site)' };
  assert.strictEqual(classer(p, SITE, [{ nom: 'ma_garde', statut: 'NON_EPROUVEE' }]).classe, 'UNKNOWN');
});

verifier('Q63 — une seule branche OR défaillante suffit à dégrader toute la policy', () => {
  // Cas réel : `... OR est_pompiste_du_jour(site) OR est_receptionniste_livraison_du_jour(site, date)`.
  // Une policy n'est jamais plus sûre que la plus faible de ses branches OR.
  const p = { table: 't', policy: 'p', cmd: 'insert', using: null,
    withCheck: "current_employee_role() = ANY (ARRAY['manager']) OR aide_saine(site) OR aide_cassee(site)" };
  const aides = [{ nom: 'aide_saine', statut: 'CONFORME' }, { nom: 'aide_cassee', statut: 'DEFAILLANTE' }];
  assert.strictEqual(classer(p, SITE, aides).classe, 'VULNERABLE',
    'une branche OR défaillante rend l’ensemble VULNERABLE, même combinée à une aide saine et à un rôle');
});

verifier('Q63 — le registre réel déclare un statut du vocabulaire clos pour chaque aide', () => {
  const declarees = aidesDeclarees(__dirname);
  assert.ok(declarees.length >= 3, 'le registre doit être peuplé');
  for (const a of declarees) {
    assert.ok(STATUTS_AIDES.includes(a.statut), `l’aide ${a.nom} doit porter un statut du vocabulaire clos (${STATUTS_AIDES.join('|')})`);
  }
  const pompiste = declarees.find(a => a.nom === 'est_pompiste_du_jour');
  assert.ok(pompiste, 'est_pompiste_du_jour doit rester déclarée pendant sa correction');
  assert.notStrictEqual(pompiste.statut, 'CONFORME',
    'la correction n’a reçu aucune preuve comportementale dans ce canal — elle ne peut pas se déclarer CONFORME sur la seule relecture du SQL');
});
