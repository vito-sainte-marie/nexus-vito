...
(function (global) {
  ...
  async function chargerReception(client,site){
    ...
    var qv=await client.from('carburant_reception_visites').select('id,date_visite,heure_fin,statut,bon_livraison_reference').eq('site',site).neq('statut','en_cours').neq('statut','annulee_doublon').order('date_visite',{ascending:false}).order('heure_fin',{ascending:false}).limit(1).maybeSingle();
