...
t('CALIBRATION — le dépôt réel reste dans un ordre de grandeur exploitable', () => {
  // Mesuré le 08/09/2026 : 8 signalements sur 62 écrans, tous relus.
  // REMESURÉ sur la ligne `production` le 16/09/2026, au rapatriement, parce
  // qu'un chiffre hérité n'est pas une mesure : 7 signalements sur 61 écrans.
  // Ce
...
t('les deux entrées du dépôt acceptent l’appel sans argument', () => {
  // `analyserDepot` avait son repli sur RACINE, `ecransDuDepot` non : l'API
  // exportée plantait sur un appel sans argument, là où sa voisine répondait.
  // Corrigé au rapatriement (16/09/2026). Les deux doivent voir le même dépôt.
  assert.strictEqual(G.ecransDuDepot().length, G.ecransDuDepot(path.join(__dirname)).length);
  assert.ok(G.ecransDuDepot().length > 0, 'la racine du dépôt doit porter des écrans NEXUS-*.html');
  assert.strictEqual(G.analyserDepot().length, G.analyserDepot(path.join(__dirname)).length);
});
